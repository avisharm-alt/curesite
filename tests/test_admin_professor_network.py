...
@pytest.mark.anyio
async def test_admin_get_all_professors_handles_legacy_timestamps(fake_db):
    fake_db.professor_network.docs.append({
        "id": "prof-legacy",
        "user_id": "legacy-user",
        "department": "History",
        "research_areas": ["Medical History"],
        "lab_description": "Explores the evolution of medical practices.",
        "accepting_students": True,
        "contact_email": "legacy.prof@example.com",
    ...
    fake_db.users.docs.append({
        ...
        "name": "Legacy Professor",
        "university": "Historical University",
        "user_type": "professor",
    })
    ...
    results = await server.admin_get_all_professors(current_user=admin_user)

    assert len(results) == 1
    profile = results[0]
    assert isinstance(profile.get("created_at"), datetime)
    assert profile["user_name"] == "Legacy Professor"
    assert profile["user_university"] == "Historical University"
