...
def _parse_datetime(value: Any) -> Any:
    """Safely coerce ISO formatted strings (including Z suffixes) into datetimes."""
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            # Leave the original value untouched when parsing fails so legacy
            # records with unexpected formats don't break API responses.
            return value
...
def parse_from_mongo(item):
    if isinstance(item, dict):
        # Work on a shallow copy so callers don't mutate the original document
        # returned by the database driver.
        parsed_item = dict(item)
        # Remove MongoDB's _id field to avoid ObjectId serialization issues
        parsed_item.pop('_id', None)

        for field in ('created_at', 'submitted_at', 'reviewed_at', 'expires_at'):
            if field in parsed_item:
                parsed_item[field] = _parse_datetime(parsed_item[field])
