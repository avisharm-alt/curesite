...
ADMIN_EMAILS = {
    email.strip().lower()
    for email in os.environ.get("ADMIN_EMAILS", "curejournal@gmail.com").split(",")
    if email.strip()
}
...
async def google_callback(request: Request):
    try:
        ...
        if existing_user:
            if existing_user.get("email", "").lower() in ADMIN_EMAILS and existing_user.get("user_type") != "admin":
                await db.users.update_one(
                    {"id": existing_user["id"]}, {"$set": {"user_type": "admin", "verified": True}}
                )
            ...
            user_type = "admin" if user_email.lower() in ADMIN_EMAILS else "student"
